...
MUTATIONS: list[tuple[str, Path, str, str, str]] = [
    ...
    (
        "a virtualenv must be able to install, not merely exist",
        SETUP,
        '    probe = subprocess.run(\n        [str(python), "-c", "import pip"], capture_output=True, timeout=60\n    )\n    return probe.returncode == 0',
        "    return True",
        "test_a_half_bootstrapped_virtualenv_does_not_count_as_one",
    ),
    (
        "the venv failure message reads stdout as well as stderr",
        SETUP,
        '        why = (made.stdout.strip() + "\\n" + made.stderr.strip()).strip()',
        "        why = made.stderr.strip()",
        "test_the_venv_failure_says_what_the_tool_said_on_stdout",
    ),
    (
        "an unusable virtualenv is cleared before recreating",
        SETUP,
        "        shutil.rmtree(VENV, ignore_errors=True)",
        "        pass",
        "test_an_unusable_virtualenv_is_cleared_rather_than_reused",
    ),
    (
        "--check gets out before the directory is touched",
        SETUP,
        "    half_made = VENV.exists()\n    if check_only:",
        "    half_made = VENV.exists()\n    if False:",
        "test_check_mode_never_deletes_a_virtualenv",
    ),
    (
        "the config is not reported out of a broken virtualenv",
        SETUP,
        "    if not venv_usable():",
